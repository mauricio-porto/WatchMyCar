...
public class AudioCodec {
	...
	public void start() throws IllegalStateException, IOException {
		if (recorder == null) {
			...
			recorder = new AudioRecord(
					...
					AudioFormat.ENCODING_PCM_16BIT,
					minSize);
			int s = recorder.getState();
			Log.d("AudioCodec", "AudioRecord state is " + s);
			int retries = 5;
			while (s != AudioRecord.STATE_INITIALIZED && retries-- > 0) {
				try {
					Thread.sleep(500);
			...
			if (s == AudioRecord.STATE_INITIALIZED) {
    ...
    public short[] getAmplitude() {
    	if (recorder != null) {
            ...
            Arrays.sort(buffer);
            //Log.e("AudioCodec", "Recorder has read: " + readBytes + " the maximum is: " + buffer[minSize-1]);
